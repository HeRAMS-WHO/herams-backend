    ...
    implements ActiveRecordHydratorInterface, ModelHydratorInterface
{
    ...
    private function castForDatabase(
        bool|float|int|string|array|object|null $complex
    ): bool|float|int|string|array|null|Expression {
    ...
    public function hydrateConstructor(Model $source, string $class): object
    {
        ...
        $args = [];
        foreach (
            $reflectionClass->getConstructor()->getParameters() as $parameter
        ) {
    ...
    private function castType(
        ReflectionNamedType $property,
        mixed $value,
        string $attribute,
        ...
        if (!$property->isBuiltin()) {
            ...
            return match (true) {
                $name === LocalizedString::class => $this->castLocalizedString(
                    $value,
                    $name
                ),
                ...
                is_subclass_of(
                    $name,
                    BackedEnum::class
                ) => $this->castBackedEnum($value, $name, $source),
                is_subclass_of($name, Enum::class) => $this->castEnum(
                    $value,
                    $name
                ),
                is_subclass_of($name, IntegerId::class) => $this->castIntegerId(
                    $value,
                    $name,
                    $property->allowsNull()
                ),
                is_subclass_of($name, StringId::class) => $this->castStringId(
                    $value,
                    $name
                ),
                is_subclass_of($name, UuidInterface::class)
                || $name === UuidInterface::class => $this->castUuid(
                    $value,
                    $name,
                    $source
                ),
                ...
                default => throw new InvalidArgumentException(
                    "Attribute $attribute has a complex type: {$property->getName()}"
                )
            };
        }
     ...
     */
    private function castLocalizedString(
        string|array $value,
        string $name
    ): LocalizedString {
    ...
    private function castBackedEnum(
        $value,
        string $name,
        HydrateSource $source
    ): BackedEnum {
     ...
     * @param  class-string  $class
     */
    private function castIntegerId(
        $value,
        string $class,
        bool $optional = false
    ): IntegerId|null {
    ...
    private function getTypeForAttribute(
        Model $model,
        string $property
    ): ReflectionNamedType {
    ...
    public function hydrateFromRequestBody(Model $model, Request $request): void
    {
        if ($request->getIsPost() || $request->getIsPut()) {
            $this->hydrateFromRequestArray(
                $model,
                $request->getBodyParams()[$model->formName()]
            );
            return;
        }
        throw new InvalidArgumentException(
            "Could not extract data from request"
        );
    }
    ...
    public function hydrateFromRequestArray(Model $model, array $data): void
    {
        foreach ($model->safeAttributes() as $attribute) {
            if (isset($data[$attribute])) {
                $model->$attribute = $this->castValue(
                    $model,
                    $attribute,
                    $data[$attribute],
                    HydrateSource::webForm
                );
            }
        }
    }

    public function hydrateFromRequestQuery(
        Model $model,
        Request $request
    ): void {
        ...
        $this->hydrateFromRequestArray(
            $model,
            $request->getQueryParams()[$model->formName()] ?? []
        );
    }
    ...
    public function hydrateFromActiveRecord(
        ActiveRecord $source,
        Model $target
    ): void {
