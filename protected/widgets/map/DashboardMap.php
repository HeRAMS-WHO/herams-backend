    ...
    private function getCollections(iterable $data)
    {
        ...
        foreach ($data as $response) {
            try {
                ...
                $workspace_url = \Yii::$app->user->can(Permission::PERMISSION_LIMESURVEY, Workspace::findOne(['id' => $response['workspace_id']])) ? Url::to(['/workspace/limesurvey', 'id' => $response['workspace_id']]) : Url::to(["/project/workspaces?Workspace[id]={$response['workspace_id']}",'id' => $this->element->page->project->id]);
