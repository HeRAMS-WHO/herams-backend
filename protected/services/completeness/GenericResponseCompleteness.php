...
class GenericResponseCompleteness implements ResponseCompletenessInterface
{
    ...
    {
        // A response without a location (GEO1) or a date is never complete.
        if ($this->isEmpty($this->scalar($data['GEO1'] ?? null)) || $this->isEmpty($date)) {
            return false;
        }
        ...
        if (!$this->skipsBuildingConditionAndFunctionality($data)) {
        ...
        if (in_array($hfacc, [self::AVAILABLE, self::PARTIAL], true)) {
            return !$this->requiresServiceEvaluation($data) || $this->hasAvailableService($data);
        }
    ...
    /**
     * When true, the CONDB and HFFUNCT clauses are skipped and evaluation
     * jumps from MoSD4 straight to HFACC. Country questionnaires that omit
     * these questions for some respondents override this.
     */
    ...
    /**
     * When false, clause 5 no longer requires at least one answered service.
     * Country questionnaires that end before the services section override this.
     */
